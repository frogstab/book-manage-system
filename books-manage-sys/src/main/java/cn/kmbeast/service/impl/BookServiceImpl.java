...
public class BookServiceImpl implements BookService {
    ...
    public Result<Void> save(Book book) {
        if (!StringUtils.hasText(book.getName())){
            return ApiResult.error("书籍名不能为空");
        }
        if (!StringUtils.hasText(book.getIsbn())){
            return ApiResult.error("ISBN不能为空");
        }
        if (!StringUtils.hasText(book.getAuthor())){
            return ApiResult.error("作者不能为空");
        }
        if (!StringUtils.hasText(book.getCover())){
            return ApiResult.error("封面不能为空");
        }
        if (book.getNum() == null){
            return ApiResult.error("馆藏数不能为空");
        }
